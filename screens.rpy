...
init -1 style edited is default:
    font "mod_assets/fonts/VerilySerifMono.otf"
    kerning 8
    outlines [(10, "#000", 0, 0)]
    xpos gui.text_xpos
...
init -1 style poemgame_text:
    yalign 0.5
    font "mod_assets/fonts/Halogen.ttf"
    size 30
    color "#000"
    outlines []
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Будь ласка, введіть своє ім'я", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("Нова гра") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Будь ласка, введіть своє ім'я", ok_action=Function(FinishEnterName)))
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Допомога") action [Help("README.html"), Show(screen="dialog", message="Файл з допомогою відкрито в вашому браузері.", ok_action=Hide("dialog"))]
...
init -1 style navigation_button_text:
    ...
    font "mod_assets/fonts/RifficFree-Bold.ttf"
    color "#fff"
...
init -1 python:
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="Помилка файла: \"characters/sayori.chr\"\n\nФайл відсутній або пошкоджений.",
                ok_action=Show(screen="dialog", message="Файл збереження пошкоджений. Початок нової гри.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="Більше немає потреби зберігатись.\nНе хвилюйся, я нікуди не піду.", ok_action=Hide("dialog"))
        else:
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "check"
                    label _("Пропуск")
                    textbutton _("Непрочатиний текст") action Preference("skip", "toggle")
                    ...
                    if config.has_sound:
                        ...
                        hbox:
                            ...
                            if config.sample_sound:
                                textbutton _("Перевірка") action Play("sound", config.sample_sound)
                    ...
                    if config.has_voice:
                        ...
                        hbox:
                            ...
                            if config.sample_voice:
                                textbutton _("Перевірка") action Play("voice", config.sample_voice)

                    if config.has_music or config.has_sound or config.has_voice:
                        null height gui.pref_spacing

                        textbutton _("Зглушити все"):
...
init -501 screen name_input(message, ok_action):
    ...
    frame:
        ...
        input default "" value VariableInputValue("player") length 12 allow "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz АБВГДЕЁЗЖИЙКЛМНОПРСТУФХЦЧШЩЫЬЪЭЮЯабвгдеёзжийклмнопрстуфхчцшщьъэюя"
